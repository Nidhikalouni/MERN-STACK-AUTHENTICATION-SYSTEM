...
const Login = () => {
  ...
  const onSubmitHandler = async (e)=>{
    ...
    try {
       ...
       axios.defaults.withCredentials = true; //it will send cookies also with the req // Allows cookies (for auth)

       if(state === 'Sign Up'){
        const {data} = await axios.post(backendURL + '/api/auth/register',
           {name,email,password}
          ) //making api call to registration api
         ...
         const {data} = await axios.post(backendURL + '/api/auth/login',
           {email,password} ) //making api call to registration api
